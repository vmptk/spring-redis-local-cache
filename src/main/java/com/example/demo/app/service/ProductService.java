...
public class ProductService {
    ...
    public Optional<Product> findProductById(ProductId productId) {
        log.info("Fetching product from repository: {}", productId.getValue());
        return productRepository.findById(productId);
    }

    @CachePut(value = "products", key = "#product.id.value")
    public Product createProduct(Product product) {
        log.info("Creating product: {}", product.getId().getValue());
        return productRepository.save(product);
    }

    @CachePut(value = "products", key = "#product.id.value")
    public Product updateProduct(Product product) {
        log.info("Updating product: {}", product.getId().getValue());
        return productRepository.save(product);
    }

    @CacheEvict(value = "products", key = "#productId.value")
    public void deleteProduct(ProductId productId) {
        log.info("Deleting product: {}", productId.getValue());
